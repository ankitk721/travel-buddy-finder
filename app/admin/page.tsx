...
export default function AdminPage() {
  ...
  const getMethodBadge = (method: string) => {
    ...
    const badge = badges[method] || { label: method, color: 'bg-stone-100 text-stone-800' }
    return (
  ...
  if (loading || !authorized) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-stone-50">
        <p className="text-xl text-stone-600">Checking authorization...</p>
      </div>
    )
  }
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        {stats && (
          <div className="grid md:grid-cols-5 gap-4 mb-8">
            <div className="bg-white rounded-xl border border-stone-200 shadow-sm p-6">
              ...
              <p className="text-3xl font-bold text-stone-900">{stats.total_contacts}</p>
            </div>
            <div className="bg-white rounded-xl border border-stone-200 shadow-sm p-6">
              <p className="text-stone-600 text-sm mb-1">WhatsApp</p>
              <p className="text-3xl font-bold text-green-600">{stats.whatsapp_count}</p>
            </div>
            <div className="bg-white rounded-xl border border-stone-200 shadow-sm p-6">
              <p className="text-stone-600 text-sm mb-1">Phone Calls</p>
              <p className="text-3xl font-bold text-blue-600">{stats.phone_count}</p>
            </div>
            <div className="bg-white rounded-xl border border-stone-200 shadow-sm p-6">
              <p className="text-stone-600 text-sm mb-1">Emails</p>
              <p className="text-3xl font-bold text-primary-600">
                {stats.email_copy_count + stats.email_send_count}
              </p>
            </div>
            <div className="bg-white rounded-xl border border-stone-200 shadow-sm p-6">
              <p className="text-stone-600 text-sm mb-1">Unique Users</p>
        ...
        <div className="bg-white rounded-xl border border-stone-200 shadow-sm">
          <div className="px-6 py-4 border-b">
            <h2 className="text-xl font-bold text-stone-900">Recent Contacts</h2>
          </div>
              ...
              <table className="w-full">
                <thead className="bg-stone-50">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-stone-500 uppercase">Method</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-stone-500 uppercase">Contacted By</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-stone-500 uppercase">Trip</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-stone-500 uppercase">Route</th>
                  </tr>
                </thead>
                    ...
                    <tr key={contact.id} className="hover:bg-stone-50">
                      ...
                      <td className="px-6 py-4">
                        <div className="text-sm">
                          ...
                          <p className="text-stone-500">{contact.contacted_by_email}</p>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="text-sm">
                          <p className="font-medium text-stone-900">{contact.traveler_name}</p>
                          <p className="text-stone-500">by {contact.trip_owner_name}</p>
                        </div>
                      </td>
